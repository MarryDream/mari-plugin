...
async function execHandle( command: string ): Promise<string> {
	return new Promise( ( resolve, reject ) => {
		exec( command, { cwd: `${ process.cwd() }/src/plugins/mari-plugin/` }, ( error, stdout, stderr ) => {
			if ( error ) {
				reject( error );
			} else {
				resolve( stdout );
...
export default defineDirective( "order", async i => {
	...
	if ( !oldDate ) {
		await i.sendMessage( "初次使用该指令，将直接尝试更新茉莉" );
		try {
			await updateBot( i );
		} catch ( error ) {
			return;
	...
	if ( commitsNew.length === 0 ) {
		await i.sendMessage( "茉莉已经是最新版了" );
		return;
	}
	
	await i.sendMessage( "开始尝试更新茉莉" );
